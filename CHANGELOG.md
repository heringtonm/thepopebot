...
**To adopt the new architecture:** Run `npx thepopebot init` in a fresh directory and run the setup wizard. Then copy over your configuration files — `config/SOUL.md`, `config/CHATBOT.md`, `config/AGENT.md`, `config/CRONS.json`, `config/TRIGGERS.json`, and any custom `.md` files you created. Move your `.pi/skills/` directory and any cron/trigger shell scripts as well. Your agent's personality, scheduled jobs, and skills carry over — only the surrounding infrastructure changes.
